...
> Эта инструкция относится только к сохранённому статическому прототипу `real-city.html`. Основной симулятор с SUMO запускается по [README проекта](../README.md). Упоминание Streamlit-зависимостей ниже описывает прежнюю версию репозитория; текущий `requirements.txt` содержит зависимости SUMO-сервера.
